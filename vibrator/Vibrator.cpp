...
static constexpr int32_t MAX_VTG_INPUT = 1740;
...
Return<void> Vibrator::perform(Effect effect, EffectStrength strength,
    ...
    if (effect == Effect::CLICK) {
        ...
        for (uint32_t i = 0; i < ARRAY_SIZE(WAVEFORM_DOUBLE_CLICK_EFFECT_SEQ); i++) {
            mBuffers[i] << std::hex <<
                static_cast<int>(WAVEFORM_DOUBLE_CLICK_EFFECT_SEQ[i]) << std::endl;
        }
        mBufferUpdate << 1 << std::endl;
